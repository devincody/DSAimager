	...
	***************************************************/
	...
	while(!EXIT_FLAG){
		...
		for (int s = 0; s < N_STREAMS; s ++){
			...
			gpuErrchk(cudaMemcpyAsync(&(d_data[(time[s] % N_TIMESTEPS)*n_floats_per_freq*n_usable_idx]), 
		...
		for (int s = 0; s < N_STREAMS; s ++){
			int img_strt = s*SZ*SZ;
			grid<<<1,45,0,stream[s]>>>(&(d_A[img_strt]),  &(d_data[(time[s] % N_TIMESTEPS)*n_floats_per_freq*n_usable_idx]), d_A_cal, d_B_cal, 250, 500, 0);
