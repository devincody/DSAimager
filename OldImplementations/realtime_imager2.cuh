...
__global__
void grid(cufftComplex* A, float* data, int most_recent, cufftComplex* A_cal, cufftComplex* B_cal, int idx_chunk_start, int idx_chunk_end, float DM){
	...
	*/
	...
	float f_low = (f_tot_end - static_cast<float>(idx_chunk_end + idx_usable_start)*bw_per_channel); // chunk end <=> low freq
	//float f_hig = (f_tot_end - static_cast<float>(idx_chunk_start + idx_usable_start)*bw_per_channel); // chunk start <=> high freq
	float time_delay_low_freq = 4.148*DM/(f_low*f_low); // ms
	...
	float f_ghz;
	int DM_OFFSET, DM_offset_time_steps;

	int f = idx_chunk_start + tid; //index of the frequency which this thread is analyzing
	while (f < idx_chunk_end){
	// for (int f = idx_chunk_start; f < idx_chunk_end; f++){
		// In this simulation, we are analyzing the top 250 frequencies
		f_ghz = (f_tot_end - static_cast<float>(f + idx_usable_start)*bw_per_channel);
		DM_offset_time_steps = floor((time_delay_low_freq - 4.148*DM/(f_ghz*f_ghz))/time_per_step);
		DM_OFFSET = ((most_recent - DM_offset_time_steps)%N_TIMESTEPS) * (n_floats_per_freq*n_usable_idx);


		wavelength = c/(1E9*f_ghz);
		
		// load the A pol data into memory
		tempa.x = data[DM_OFFSET + f*220 + 4*baseline_number];
		tempa.y = data[DM_OFFSET + f*220 + 4*baseline_number+1];


		// load the B pol data into memory
		tempb.x = data[DM_OFFSET + f*220 + 4*baseline_number+2];
		tempb.y = data[DM_OFFSET + f*220 + 4*baseline_number+3];
