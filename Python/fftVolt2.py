'''
All options work to some degree. Fake data imaging seems to work reliably.

Using actual data works somewhat, it's best to use voltages to obtain the
cross correlations. It appears that using the cross-correlations as
stored in the matrix do not work yet. 
...
def main():
	...
	if use_crab_data:
		use_DSA_positions = True
		# f_start = 200; f_end = 1848
		f_start = 300; f_end = 1800

		# time_list = range(200,270)
		time_list = [231]
	else:
	...
	for time_n in time_list:
				...
				for k in range(antennas):
					for j in range(f_samples): #iterate through frequencies
						...
						if use_voltages_for_vis:
							...
							# if (i == 3 and k == 1 and j > 1645):
							# 	print("antorder = {} & {}".format(np.floor(mv*(pos[ant_ord[i]-1,0]-pos[ant_ord[k]-1,0])/wavelength), np.floor(mv*(pos[ant_ord[i]-1,1]-pos[ant_ord[k]-1,1])/wavelength)))
							# 	print("antorder = {} & {}".format(mv*(pos[ant_ord[i]-1,0]-pos[ant_ord[k]-1,0])/wavelength, mv*(pos[ant_ord[i]-1,1]-pos[ant_ord[k]-1,1])/wavelength))
							# 	print("i = {}, j = {}, f = {}, cor = {}". format(i,k,wavelength	, cor))
							# 	print("x = {}, y = {}, idx = {}".format(x,y, idx))
							# 	print("f = {}".format(wavelength))
		...
		if (not use_visibilities and use_crab_data):
			...
			print("time_n ={}, min = {}, max = {}". format(time_n, np.min(C), np.max(C)))
			plt.imshow(C, cmap = "hot", extent=[-float(sz)/2*sm_k,float(sz)/2*sm_k,float(sz)/2*sm_k,-float(sz)/2*sm_k], vmax = 70000)
		else:
		...
		if use_crab_data:
			...
			plt.colorbar()
			plt.savefig("frames2/" + str(time_n).zfill(4) + ".png")
			# plt.show()
			plt.close()
		else:
